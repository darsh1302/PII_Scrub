"""Guardrail G17 â€” chunk boundaries must not lose long secrets.
...
200 characters of overlap, a private key is present in neither chunk â€” detected
...
from pii_agent.core.chunker import chunk_text, reconstruct, resolve_overlap
...
def test_pem_key_detected_exactly_once_when_straddling_a_boundary():
    ...
    # begins â€” the chunk reaches back by the overlap.
...
def test_small_overlap_demonstrates_the_original_defect():
    ...
    in neither chunk, so it is detected nowhere â€” silently.
    ...
    """
    import pii_agent.core.chunker as chunker_module
...
def test_corrected_overlap_finds_the_same_key_the_small_overlap_lost():
    ...
    Same text, same chunk size â€” only the overlap differs.
    """
    profile = resolve_profile("DEFAULT_PII")
    chunk_size = 16384
    text = _text_with_key_at(chunk_size - 400)
...
def test_key_found_at_every_position_relative_to_a_boundary(offset):
    """Sweep the key across boundary positions â€” no position may lose it."""
    profile = resolve_profile("DEFAULT_PII")
...
# ---------------------------------------------------------------------------
# Chunking correctness â€” coverage figures are meaningless without this
# ---------------------------------------------------------------------------
...
def test_chunks_prefer_line_boundaries():
    ...
    starts â€” the chunk reaches back by the overlap.
    """
