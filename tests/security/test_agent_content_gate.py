...
from agent.memory import prepare_for_model, redact_for_storage, SessionMemory
from agent.state import initial_state
from session.context import get_session_context
from tools import ForbiddenCapability, build_registry
from utils.config import Settings
from utils.content_gate import (
    GateViolation,
    assert_gate_safe,
    gate_tool_output,
    sanitize_error,
...
from utils.prompt_safety import PromptSafety
...
def test_registry_rejects_a_forbidden_tool_if_one_is_added(session, monkeypatch):
    ...
    from tools.agent_tools import _SessionTool
...
def test_tool_calls_survive_redaction():
    ...
    from agent.memory import redact_message
...
def test_bare_name_resolves_to_an_upload(session):
    ...
    from tools.agent_tools import _resolve_source
